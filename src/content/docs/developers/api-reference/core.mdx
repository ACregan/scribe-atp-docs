...
Thrown when a request reached the PDS but it responded with a non-ok HTTP status. The service is up — this specific operation failed. Safe to retry — see [`withRetry`](#withretry) and the [Errors and retries](/developers/guides/errors-and-retries/) guide.
