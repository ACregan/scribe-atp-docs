...
Derives the fully-qualified canonical URL for an article. Uses `article.canonicalUrl` if set; otherwise combines `article.site` (the site's HTTPS URL), `site.urlPrefix`, and `article.path`.
...
interface Article {
  ...
  site: string;            // HTTPS URL of the canonical site, e.g. "https://myblog.com"
