...
A React button that creates an AT Protocol `site.standard.graph.recommend` record ("recommend") for an article when clicked. Opens a popup OAuth flow at `social.scribe-atp.app`, then updates to a confirmed state on success.
...
All four storage functions above are SSR-safe — `localStorage` access is wrapped in a try/catch. They return `false` / do nothing when called in environments where `localStorage` is unavailable.

<Aside type="note">
  A fifth function, `clearSubscribed`, exists internally and is called automatically by `SubscribeButton` after a confirmed unsubscription — but it isn't exported from the package's public API. If you're building your own unsubscribe UI outside `SubscribeButton`, note that `markSubscribed` won't help here (it only sets the subscribed state, not clears it) — you'll need to track unsubscribed state with your own `localStorage` key for now.
</Aside>
