...
Fetching a Site gives you the full list of articles without making additional requests — because the Site record contains **ArticleRefs**: lightweight cached snapshots of each article's metadata (title, slug, description, splash image). Only the full `content` field is excluded.
