...
| `site.standard.document` | Published article records |
| `app.scribe.article` | Draft article records (not yet published) |

Each record within a collection has an **rkey** (record key) — a unique identifier within that collection. For Scribe, article rkeys are slugs (e.g. `my-first-post`). Publication rkeys are opaque TIDs (e.g. `3mp4nd46xwr2h`) assigned by the AT Protocol.
...
at://did:plc:abc123/site.standard.document/my-first-post
```
...
An **Article** is a single piece of written content. Published articles are stored in the `site.standard.document` collection; drafts (not yet published) live in `app.scribe.article`. It contains:
- `title`, `description`, `content` (full HTML), `path` (full URL path), `splashImageUrl`
...
- `site` — AT URI of the canonical `site.standard.publication` record
- `canonicalUrl` — fully-qualified URL, e.g. `https://myblog.com/blog/creative-writing/my-article`
- `createdAt`, `publishedAt`, `updatedAt` timestamps
