...
An **Article** is a single piece of written content. All articles — whether draft, unpublished, or published — are stored in the `site.standard.document` collection. An article's publication state is determined entirely by whether it appears in a Site record (see [Publication states](#publication-states) below), not by which collection it lives in. It contains:
